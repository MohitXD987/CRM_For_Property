document.addEventListener('DOMContentLoaded', async function () {
    ...
    const CONFIG = {
        WHATSAPP_TOKEN: config.getToken(),
        PHONE_NUMBER_ID: '1002392779614958',
        WABA_ID: config.getWabaId(),
        API_VERSION: config.getApiVersion()
    };
    
    if (!CONFIG.WHATSAPP_TOKEN || !CONFIG.WABA_ID) {
        console.error('Missing configuration. Check .env file.');
        showToast('Configuration error. Check console.', 'error');
    }
